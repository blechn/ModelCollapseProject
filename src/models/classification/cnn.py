...
class CNNClassifier(L.LightningModule):
    ...
    def get_features(self, x):
        features = self.model.layers[:-2](
            x
        )  # get the features in 64 dimensional space for a nice representation
        return features
...
def get_trained(**kwargs):
    ...
    if kwargs.get("train", False):
        ...
        logger_add += "_fashion_"
        print("Using FashionMNIST") if kwargs.get("fashion") else ""
        trainer = L.Trainer(
            logger=TensorBoardLogger(path, "cnnclassifier_logs"),
        ...
        if kwargs.get("fashion"):
            model = CNNClassifier.load_from_checkpoint(
                path
                / Path(
                    "cnnclassifier_logs/version_15/checkpoints/epoch=4-step=4690.ckpt"
                )
            )  # hardcoded, good model for FashionMNIST
        else:
            model = CNNClassifier.load_from_checkpoint(
                path
                / Path(
                    "cnnclassifier_logs/version_5/checkpoints/epoch=4-step=4690.ckpt"
                )
            )  # hardcoded, good model for MNIST
