    ...
    def sample(self, n_samples: int = 1, batch_size: int = 1024, device=None, **kwargs):
        ...
        y_full = (
            torch.nn.functional.one_hot(
                torch.arange(0, condition_size, device=device).repeat_interleave(n_samples)
            )
        ).float()
        ...
        for i in trange(0, total_to_generate, batch_size, desc="Generating data", leave=True):
        ...
        return torch.cat(decoded_samples, dim=0).to(device), y_full.to(device) # create tensor from all generated batches
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", action="store_true", default=False, help="Whether to train a model or just load it.")
    parser.add_argument("--max_epochs", action="store", type=int, default=10, help="How many epochs to train.")
